...
#ifndef TEST_DIVIDEANDCONQUERMAPPERCONTROLLER
#define TEST_DIVIDEANDCONQUERMAPPERCONTROLLER
#endif
...
int RunDividedAndConquerMapper(int argc,char** argv)
{
    using namespace DACSFM;
    DividedAndConquerMapperController::Options dividedandconquer_options;
    ...
    OptionManager options;
    options.AddRequiredOption("workspace_path",
                                &dividedandconquer_options.workspace_path);
    options.AddRequiredOption("image_path", &dividedandconquer_options.image_path);
    options.AddDefaultOption("mask_path", &dividedandconquer_options.mask_path);
    options.AddDefaultOption("vocab_tree_path",
                            &dividedandconquer_options.vocab_tree_path);
    options.AddDefaultOption("data_type", &data_type,
                            "{individual, video, internet}");
    options.AddDefaultOption("quality", &quality, "{low, medium, high, extreme}");
    options.AddDefaultOption("camera_model",
                            &dividedandconquer_options.camera_model);
    options.AddDefaultOption("single_camera",
                            &dividedandconquer_options.single_camera);
    options.AddDefaultOption("sparse", &dividedandconquer_options.sparse);
    options.AddDefaultOption("dense", &dividedandconquer_options.dense);
    options.AddDefaultOption("mesher", &mesher, "{poisson, delaunay}");
    options.AddDefaultOption("num_threads", &dividedandconquer_options.num_threads);
    options.AddDefaultOption("use_gpu", &dividedandconquer_options.use_gpu);
    options.AddDefaultOption("gpu_index", &dividedandconquer_options.gpu_index);
    options.Parse(argc, argv);

    StringToLower(&data_type);
    if (data_type == "individual") {
        dividedandconquer_options.data_type =
            AutomaticReconstructionController::DataType::INDIVIDUAL;
    } else if (data_type == "video") {
        dividedandconquer_options.data_type =
            AutomaticReconstructionController::DataType::VIDEO;
    ...
    StringToLower(&quality);
    if (quality == "low") {
        dividedandconquer_options.quality =
            AutomaticReconstructionController::Quality::LOW;
    } else if (quality == "medium") {
        dividedandconquer_options.quality =
            AutomaticReconstructionController::Quality::MEDIUM;
    ...
    StringToLower(&mesher);
    if (mesher == "poisson") {
        dividedandconquer_options.mesher =
            AutomaticReconstructionController::Mesher::POISSON;
    } else if (mesher == "delaunay") {
        dividedandconquer_options.mesher =
            AutomaticReconstructionController::Mesher::DELAUNAY;
    ...
    ReconstructionManager reconstruction_manager;
    SceneCut::Options clustering_options;
    clustering_options.max_num_images = 100;

    clustering_options.image_overlap = 50;

    clustering_options.overlap_ratio = 0.5;

    clustering_options.relax_retio = 1.3;

    clustering_options.clusters_partitioned = 2;
    clustering_options.output_path = JoinPaths(dividedandconquer_options.workspace_path,"SceneCut");

    DividedAndConquerMapperController controller(dividedandconquer_options,clustering_options,
                                                    &reconstruction_manager);

    reconstruction_manager.Write(,&options);
    controller.Start();
    controller.Wait();
...
int main(int argc, char** argv) {
  ...
  commands.emplace_back("dividedandconquer_mapper", &RunDividedAndConquerMapper);
